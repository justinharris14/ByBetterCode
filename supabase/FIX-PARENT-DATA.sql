-- ========================================
-- FIX PARENT DATA - ADD CHILDREN & TEST DATA
-- ========================================
-- This will add children and test data for existing parents
...
SELECT '=== VERIFICATION: PARENT-CHILD RELATIONSHIPS ===' AS section;
SELECT 
  u.first_name || ' ' || u.last_name AS parent,
  u.email,
  c.first_name || ' ' || c.last_name AS child,
  EXTRACT(YEAR FROM AGE(c.dob)) || ' years old' AS age
FROM public.users u
JOIN children c ON c.parent_id = u.user_id
WHERE u.role = 'parent'
...
BEGIN
  ...
  📱 Parents will now see in the app:
